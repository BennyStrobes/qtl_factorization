...
def annot_parser_return_annotation_indices(fh, compression, frqfile_full=None, compression_frq=None):
	'''Parse annot files'''
	df_annot = read_csv(fh, header=0, compression=compression).drop(['SNP','CHR', 'BP', 'CM'], axis=1, errors='ignore').astype(float)
	if frqfile_full is not None:
		df_frq = frq_parser(frqfile_full, compression_frq)
		indices = (.95 > df_frq.FRQ) & (df_frq.FRQ > 0.05)
		df_annot = df_annot[indices]
	return df_annot, indices
...
def annot_return_annotations_and_index(fh_list, num=None, frqfile=None):
	'''
	...
	if num is not None:  # 22 files, one for each chromosome
		...
		for chrom in chrs:
			if frqfile is not None:
				df_annot_chr_list, indices = annot_parser_return_annotation_indices(sub_chr(fh_list[0], chrom) + annot_suffix[0], annot_compression[0], sub_chr(frqfile, chrom) + frq_suffix, frq_compression)

			else:
				print('should not be here')
				pdb.set_trace()

			annot_matrix_chr = np.matrix(df_annot_chr_list)
			y.append(np.dot(annot_matrix_chr.T, annot_matrix_chr))
			M_tot += annot_matrix_chr.shape[0]
			annot_arr.append(annot_matrix_chr)
			indices_arr.append(indices)

		x = sum(y)
		annot_arr = np.vstack(annot_arr)
		indices_arr = np.hstack(indices_arr)
		# Quick error checking
		if np.sum(indices_arr) != annot_arr.shape[0]:
			print('assumption eroror')
			pdb.set_trace()
	else:  # just one file
		print('assumption error')
		print('not implemented for this setting')
		pdb.set_trace()

	return x, M_tot, annot_arr, indices_arr
