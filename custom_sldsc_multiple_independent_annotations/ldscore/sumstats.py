...
def _read_ld_sumstats_and_return_indices_mapping_ref_snps_to_sumstats_snps(args, log, fh, alleles=False, dropna=True):
	sumstats = _read_sumstats(args, log, fh, alleles=alleles, dropna=dropna)
	ref_ld = _read_ref_ld(args, log)
	n_annot = len(ref_ld.columns) - 1
	M_annot = _read_M(args, log, n_annot)
	...
	sumstats = _merge_and_log(ref_ld, sumstats, 'reference panel LD', log)
	sumstats = _merge_and_log(sumstats, w_ld, 'regression SNP LD', log)
	ordered_ref_ld_snps = np.asarray(ref_ld['SNP'])
	ordered_sumstat_snps = np.asarray(sumstats['SNP'])
	# Dictionary from value to position ordered_ref_ld_snps
	dicti = {}
	for i,val in enumerate(ordered_ref_ld_snps):
		dicti[val] = i
	indices = []
	for snp_id in ordered_sumstat_snps:
		indices.append(dicti[snp_id])
	indices = np.asarray(indices)
	if np.array_equal(ordered_ref_ld_snps[indices], ordered_sumstat_snps) == False:
		print('assumption erroror')
		pdb.set_trace()
	w_ld_cname = sumstats.columns[-1]
	ref_ld_cnames = ref_ld.columns[1:len(ref_ld.columns)]
	return M_annot, w_ld_cname, ref_ld_cnames, sumstats, novar_cols, indices
...
def estimate_h2_for_additional_single_annotation(args, log, M_annot, w_ld_cname, ref_ld_cnames, sumstats, novar_cols, extra_annotation_name, extra_annotation_values_npy_file, extra_annotation_ldscore_values_npy_file, extra_annotation_M_npy_file, extra_annotation_M_5_50_npy_file, indices_mapping_ref_snps_to_sumstat_snps, baseline_anno_overlap_matrix, M_tot, baseline_annot, annotation_indices):
	# Load in data for annotation
	new_anno_ld_scores = np.load(extra_annotation_ldscore_values_npy_file)
	new_anno_ld_scores = new_anno_ld_scores[indices_mapping_ref_snps_to_sumstat_snps]
	extra_anno_m_5_50 = np.load(extra_annotation_M_5_50_npy_file)
	new_anno = np.load(extra_annotation_values_npy_file)
	new_anno = new_anno[annotation_indices]

	# Add new annotation to M_annot, w_ld_cname, ref_ld_cnames, sumstats, novar_cols
	# w_ld_cname is fine
	ref_ld_cnames = ref_ld_cnames.insert(len(ref_ld_cnames),extra_annotation_name)
	sumstats[extra_annotation_name] = new_anno_ld_scores
	novar_cols = novar_cols.append(pd.Series([False], index=[extra_annotation_name]))
	M_annot = np.hstack((M_annot,np.asmatrix([extra_anno_m_5_50*1.0])))

	# Update overlap matrix
	num_baseline_anno = baseline_anno_overlap_matrix.shape[0]
	overlap_matrix = np.zeros(((num_baseline_anno+1), (num_baseline_anno+1)))
	overlap_matrix[:num_baseline_anno,:num_baseline_anno] = baseline_anno_overlap_matrix

	tmp = np.dot(baseline_annot.T, np.asmatrix(new_anno).T)
	identity_tmp = np.dot(new_anno,new_anno)
	final_tmp = np.hstack((tmp.T, np.asmatrix([identity_tmp]).T))
	overlap_matrix[-1,:] = final_tmp[0,:]
	overlap_matrix[:, -1] = final_tmp[0,:]

	# Now run standard S-LDSC procedure
	...
	single_anno_log_file = args.out + '_' + extra_annotation_name +'.log'
	t = open(single_anno_log_file,'w')
	t.write(hsqhat.summary(ref_ld_cnames, P=args.samp_prev, K=args.pop_prev, overlap = args.overlap_annot))
	t.close()
	...
	if args.overlap_annot:
		...
		df_results.to_csv(args.out + '_' + extra_annotation_name +'.results', sep="\t", index=False)
		log.log('Results printed to '+args.out+ '_' + extra_annotation_name +'.results')

def estimate_h2_for_additional_multiple_single_annotations_independently(args, log):
	...
	if args.no_intercept:
		args.intercept_h2 = 1
	# Load in baseline annotations
	M_annot, w_ld_cname, ref_ld_cnames, sumstats, novar_cols, indices_mapping_ref_snps_to_sumstat_snps = _read_ld_sumstats_and_return_indices_mapping_ref_snps_to_sumstats_snps(args, log, args.h2)

	# Load in overlap matrix for LD annotations

	#overlap_matrixa, M_tota = _read_chr_split_files(args.ref_ld_chr, args.ref_ld, log, 'annot matrix', ps.annot, frqfile=args.frqfile_chr)
	#overlap_matrix, M_tot = _read_annot(args, log)
	baseline_anno_overlap_matrix, M_tot, baseline_annot, annotation_indices = ps.annot_return_annotations_and_index(_splitp(args.ref_ld_chr), _N_CHR, frqfile=args.frqfile_chr)

	# Here we are going to do seperate S-LDSC for each additional annotation
	# First, loop through each additional annotation
	f = open(args.multi_single_anno)
	for line in f:
		line = line.rstrip()
		data = line.split('\t')
		# Each line is annotation
		# Extract relevent info for this annotation
		# Name of annotation
		extra_annotation_name = data[0]
		# npy file containing annotation values across all snps
		extra_annotation_values_npy_file = data[1]
		# npy file containing ld score of annotaiton values across all snps
		extra_annotation_ldscore_values_npy_file = data[2]
		# npy file containing M values of new annotation
		extra_annotation_M_npy_file = data[3]
		# npy file containing M_5_50 values of new annotation
		extra_annotation_M_5_50_npy_file = data[4]
		estimate_h2_for_additional_single_annotation(args, log, M_annot, w_ld_cname, ref_ld_cnames, sumstats, novar_cols, extra_annotation_name, extra_annotation_values_npy_file, extra_annotation_ldscore_values_npy_file, extra_annotation_M_npy_file, extra_annotation_M_5_50_npy_file, indices_mapping_ref_snps_to_sumstat_snps, baseline_anno_overlap_matrix, M_tot, baseline_annot, annotation_indices)
		sumstats = sumstats.drop(extra_annotation_name, axis=1)
	f.close()
