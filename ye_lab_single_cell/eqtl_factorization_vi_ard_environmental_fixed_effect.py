...
def outside_update_U_n(U_mu, U_var, G_slice, G_fe_slice, Y_slice, K, V_S_expected_val, V_S_squared_expected_val, L_expected_val, L_squared_expected_val, F_S_expected_val, covariate_predicted_slice, gamma_u, tau_expected_val, alpha_i_expected_val):
	for k in range(K):
		...
		other_environmental_fe_expected = (U_S_expected_val@L_expected_val) - U_S_expected_val[k]*L_expected_val[k,:]

		# Update variance of q(U|s=1)
		temp = (np.square(G_slice)*V_S_squared_expected_val[k,:]) + (L_squared_expected_val[k,:]) + (2.0*G_slice*V_k_S_k_expected_val*L_expected_val[k,:])
		a_term = np.sum(tau_expected_val*temp) + gamma_u[k]
		U_var[k] = 1.0/a_term
		# Update mean of q(U|s=1)
		resid = Y_slice - covariate_predicted_slice - alpha_i_expected_val - G_fe_slice*F_S_expected_val - G_slice*other_components_expected - other_environmental_fe_expected
		b_term = np.sum(tau_expected_val*(G_slice*V_k_S_k_expected_val + L_expected_val[k,:])*resid)
		U_mu[k] = U_var[k]*b_term
	return np.hstack((U_mu, U_var))

def outside_update_V_t(V_mu, V_var, G_slice, G_fe_slice, Y_slice, K, U_S_expected_val, U_S_squared_expected_val, L_t_expected_val, F_S_t_expected_val, covariate_predicted_slice, alpha_t_mu, gamma_v, tau_t_expected_val):
	for k in range(K):
		# Compute expectations on other components
		other_components_expected = (U_S_expected_val@V_mu) - U_S_expected_val[:, k]*V_mu[k]
		other_environmental_fe_expected = (U_S_expected_val@L_t_expected_val) - U_S_expected_val[:, k]*L_t_expected_val[k]
		# Update variance of q(V|s=1)
		a_term = gamma_v + (tau_t_expected_val*np.sum(np.square(G_slice)*U_S_squared_expected_val[:,k]))
		# Update mean of q(U|s=1)
		resid = Y_slice - alpha_t_mu - covariate_predicted_slice - G_slice*other_components_expected - G_fe_slice*F_S_t_expected_val - other_environmental_fe_expected
		b_term = np.sum(tau_t_expected_val*G_slice*(U_S_expected_val[:,k]*resid - U_S_squared_expected_val[:,k]*L_t_expected_val[k]))
...
def outside_update_L_t(L_t_expected_val, L_var, G_slice, G_fe_slice, Y_slice, K, U_S_expected_val, U_S_squared_expected_val, V_mu, F_S_t_expected_val, covariate_predicted_slice, alpha_t_mu, gamma_v, tau_t_expected_val):
	for k in range(K):
		# Compute expectations on other components
		other_components_expected = (U_S_expected_val@V_mu) - U_S_expected_val[:, k]*V_mu[k]
		other_environmental_fe_expected = (U_S_expected_val@L_t_expected_val) - U_S_expected_val[:, k]*L_t_expected_val[k]
		...
		# Update mean of q(U|s=1)
		resid = Y_slice - alpha_t_mu - covariate_predicted_slice - G_slice*other_components_expected - G_fe_slice*F_S_t_expected_val - other_environmental_fe_expected
		b_term = np.sum(tau_t_expected_val*(U_S_expected_val[:,k]*resid - U_S_squared_expected_val[:,k]*G_slice*V_mu[k]))

		new_var = 1.0/a_term
		new_mu = new_var*b_term
		L_var[k] = new_var
		L_t_expected_val[k] = new_mu
	return np.hstack((L_t_expected_val, L_var))
...
def outside_update_C_t(C_t_mu, C_t_var, G_slice, G_fe_slice, Y_slice, N, U_S_expected_val, V_S_t_expected_val, F_S_t_expected_val, environment_predicted_t, tau_t_expected_val, alpha_t_mu, cov, cov_squared):
	num_cov = len(C_t_mu)
	components_expected = U_S_expected_val@V_S_t_expected_val
	base_resid = Y_slice - alpha_t_mu - G_fe_slice*F_S_t_expected_val - G_slice*components_expected - environment_predicted_t
...
def outside_update_tau_t(tau_alpha, tau_beta, G_slice, G_fe_slice, Y_slice, N, U_S, V_S_t, F_S_t, C_t, L_t, V_S_t_squared, F_S_t_squared, U_S_squared, C_t_squared, L_t_squared, alpha_mu_t, alpha_var_t, cov, cov_squared, alpha_prior, beta_prior):
	# Compute Relevent expectations
	...
	squared_environment_terms = U_S_squared@L_t_squared
	environment_terms =U_S@L_t

	# First add together square terms
	resid = np.square(Y_slice) + alpha_t_squared + squared_covariate_terms + np.square(G_fe_slice)*F_S_t_squared + np.square(G_slice)*squared_factor_terms + squared_environment_terms
	# Now add terms with Y
	resid = resid - (2.0*Y_slice*(environment_terms + covariate_terms + alpha_mu_t + G_slice*factor_terms + G_fe_slice*F_S_t))

	resid = resid + 2.0*environment_terms*(covariate_terms + alpha_mu_t + G_slice*factor_terms +G_fe_slice*F_S_t)
	
	temp = np.zeros(len(resid))
	temp_k = U_S.shape[1]
	for kk in range(temp_k):
		temp = temp + 2.0*U_S_squared[:,kk]*G_slice*L_t[kk]*V_S_t[kk] - 2.0*np.square(U_S[:,kk])*G_slice*L_t[kk]*V_S_t[kk]
	resid = resid + temp
	#resid = resid + 2.0*environment_terms*G_slice*factor_terms
	...
	resid = resid + (environment_terms*environment_terms - np.sum(np.square(U_S*L_t),axis=1))
...
class EQTL_FACTORIZATION_VI(object):
	...
	def fit(self, G, G_fe, Y, z, cov):
		...
		for vi_iter in range(self.max_iter):
			...
			if np.mod(vi_iter, 5) == 0 and vi_iter > 0:
				...
				np.save(self.output_root + 'temper_L.npy', (self.L_mu))
	...
	def update_V(self):
		...
		for test_index in range(self.T):
			V_update_data.append(outside_update_V_t(V_mu_copy[:, test_index], V_var_copy[:, test_index], self.G[:, test_index], self.G_fe[:, test_index], self.Y[:, test_index], self.K, U_S_expected_val, U_S_squared_expected_val, self.L_mu[:, test_index], self.F_mu[test_index], covariate_predicted[:, test_index], self.alpha_big_mu[:, test_index], self.gamma_v, tau_expected_val[test_index]))
	...
	def update_L(self):
		...
		U_S_squared_expected_val = (np.square(self.U_mu) + self.U_var)
		tau_expected_val = self.tau_alpha/self.tau_beta
		L_mu_copy = np.copy(self.L_mu)
		L_var_copy = np.copy(self.L_var)
		covariate_predicted = np.dot(self.cov, self.C_mu)
		...
		for test_index in range(self.T):
			L_update_data.append(outside_update_L_t(L_mu_copy[:, test_index], L_var_copy[:, test_index], self.G[:, test_index], self.G_fe[:, test_index], self.Y[:, test_index], self.K, U_S_expected_val, U_S_squared_expected_val, self.V_mu[:, test_index], self.F_mu[test_index], covariate_predicted[:, test_index], self.alpha_big_mu[:, test_index], self.gamma_v, tau_expected_val[test_index]))

		# Convert to array
		L_update_data = np.asarray(L_update_data).T
		# Fill in data structures
		self.L_mu = L_update_data[(self.K*0):(1*self.K), :]
		self.L_var = L_update_data[(self.K*1):(2*self.K), :]
	...
	def update_U(self):
		...
		L_squared_expected_val = (np.square(self.L_mu) + self.L_var)
	...
	def update_F(self):
		...
		covariate_predicted = np.dot(self.cov, self.C_mu) + np.dot(self.U_mu, self.L_mu)
		F_update_data = []
		gamma_f = 0.001
		for test_index in range(self.T):
	...
	def update_C(self):
		...
		environment_predicted = np.dot(self.U_mu, self.L_mu)


		C_update_data = []
		for test_index in range(self.T):
			C_update_data.append(outside_update_C_t(C_mu_copy[:, test_index], C_var_copy[:, test_index], self.G[:, test_index], self.G_fe[:, test_index], self.Y[:, test_index], self.N, U_S_expected_val, self.V_mu[:,test_index], self.F_mu[test_index], environment_predicted[:, test_index], tau_expected_val[test_index], self.alpha_big_mu[:, test_index], self.cov, self.cov_squared))
	...
	def initialize_variables(self):
		...
		# Fixed effect for U
		self.L_mu = np.zeros((self.K, self.T))
		self.L_var = np.ones((self.K, self.T))
