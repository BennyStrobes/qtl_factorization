...
for chrom_num in range(1,23):
	...
	surge_anno_file = sldsc_processed_data_dir + 'surge_egenes_real_valued_' + fdr + '.' + str(chrom_num) + '.annot'
	joint_surge_anno_file = sldsc_processed_data_dir + 'joint_surge_egenes_real_valued_' + fdr + '.' + str(chrom_num) + '.annot'
	...
	for line in f:
		...
		for latent_factor in range(1,11):
			if variant_name_short in surge_eqtls[latent_factor]:
				t_stat = surge_eqtls[latent_factor][variant_name_short]
				joint_binary = np.max((joint_binary, t_stat))
				t.write('\t' + str(t_stat))
			else:
				t.write('\t' + '0.0')
		t_joint.write('\t' + str(joint_binary))
		if variant_name_short in standard_eqtls:
			t_stat = standard_eqtls[variant_name_short]
			t.write('\t' + str(t_stat) + '\n')
			t_joint.write('\t' + str(t_stat) + '\n')
		else:
			t.write('\t' + '0.0' + '\n')
			t_joint.write('\t' + '0.0' + '\n')
