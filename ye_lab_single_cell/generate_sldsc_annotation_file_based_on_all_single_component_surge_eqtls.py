...
for chrom_num in range(1,23):
	...
	surge_anno_file = sldsc_processed_data_dir + 'surge_' + str(component_num) + '_eqtls_' + str(pvalue_thresh) + '.' + str(chrom_num) + '.annot'
	...
	for line in f:
		...
		if head_count == 0:
			head_count = head_count + 1
			# print header
			t.write('\t'.join(data[:4]) + '\t' + 'surge_eqtl_' + str(component_num) + '\n')
			continue
		# error checking
		if len(data) != 57:
		...
		# Print header
		t.write('\t'.join(data[:4]))
		if variant_name_short in surge_eqtls[component_num]:
			t.write('\t' + '1.0' + '\n')
		else:
			t.write('\t' + '0.0' + '\n')
