...
class EQTL_FACTORIZATION_VI(object):
	...
	def update_V(self):
		...
		for test_index in range(self.T):
			V_update_data.append(outside_update_V_t(V_mu_copy[:, test_index], V_var_copy[:, test_index], self.G[:, test_index], self.Y[:, test_index], self.K, U_S_expected_val, U_S_squared_expected_val, self.F_mu[test_index], covariate_predicted[:, test_index], self.alpha_big_mu[:, test_index], gamma_v, tau_expected_val[test_index]))
	...
	def update_gamma_V(self):
		...
		for k in range(self.K):
			...
			self.gamma_V_alpha[k] = 1e-16 + (self.T/2.0)
			self.gamma_V_beta[k] = 1e-16 + (np.sum(V_squared_k_expected_val)/2.0)
