...
run_eqtl_lmm <- function(expr, geno, covariates, lfs, groups) {
	fit_full <- lmer(expr ~ geno + covariates + lfs + (1 | groups), REML=FALSE)
	fit_null <- lmer(expr ~ covariates + lfs + (1 | groups), REML=FALSE)
	...
	return(list(eqtl_pvalue=aggregate_pvalue, effect_size=effect_size, std_err=std_err))
}
...
while(!stop) {
	...
	# Unpack data
		...
		tryCatch(
		{
			lmm_results = run_eqtl_lmm(expr, geno, covariates, lfs, groups)
			#lmm_results_perm = run_lf_interaction_eqtl_lmm_perm(expr, geno, covariates, lfs, groups, individual_groups)

			new_line <- paste0(rs_id, "\t", ensamble_id ,"\t",lmm_results$eqtl_pvalue, "\t", lmm_results$effect_size, "\t", lmm_results$std_err, "\n")
        		
        	cat(new_line)
        },
        error = function(e) {
        	new_line <- paste0(rs_id, "\t", ensamble_id ,"\t", 1.0, "\t", 0.0, "\t", 1.0, "\n")
        	cat(new_line)
        }
        )
	}
