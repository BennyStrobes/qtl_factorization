...
def outside_update_U_n(U_mu, U_var, G_slice, G_fe_slice, Y_slice, K, V_S_expected_val, V_S_squared_expected_val, F_S_expected_val, covariate_predicted_slice, gamma_u, tau_expected_val, alpha_i_expected_val):
	base_resid = Y_slice - covariate_predicted_slice - alpha_i_expected_val - G_fe_slice*F_S_expected_val
	precomputed_a_terms = V_S_squared_expected_val@(tau_expected_val*np.square(G_slice))
	# Compute relevent expectations
	U_S_expected_val = U_mu
	# Compute expectations on other components (!= k)
	other_components_expected = (U_S_expected_val@V_S_expected_val) - U_S_expected_val[0]*V_S_expected_val[0,:]

	# Loop through latent contexts
	for k in range(K):
		# Update variance of q(U) according to VI update
		a_term = precomputed_a_terms[k] + gamma_u[k]
		U_var[k] = 1.0/a_term
		# Update mean of q(U) according to VI update
		resid = base_resid - G_slice*other_components_expected
		b_term = np.sum(tau_expected_val*G_slice*(V_S_expected_val[k,:])*resid)
		...
		if (k+1) < K:
			...
			U_S_expected_val = U_mu
			# Compute expectations on other components (!= k)
			other_components_expected = other_components_expected + U_S_expected_val[k]*V_S_expected_val[k,:] - U_S_expected_val[(k+1)]*V_S_expected_val[(k+1),:]
...
def outside_update_V_t(V_mu, V_var, G_slice, G_fe_slice, Y_slice, K, U_S_expected_val, U_S_squared_expected_val, F_S_t_expected_val, covariate_predicted_slice, alpha_t_mu, gamma_v, tau_t_expected_val):
	base_resid = Y_slice - alpha_t_mu - covariate_predicted_slice - G_fe_slice*F_S_t_expected_val
	precomputed_a_terms = tau_t_expected_val*((np.square(G_slice)@U_S_squared_expected_val))

	# Compute expectations on other components (!= k)
	other_components_expected = (U_S_expected_val@V_mu) - U_S_expected_val[:, 0]*V_mu[0]
	# Loop through latent contexts
	for k in range(K):
		# Update variance of q(V) according to VI update
		a_term = gamma_v + precomputed_a_terms[k]
		# Update mean of q(U) according to VI update
		resid = base_resid - G_slice*other_components_expected
		...
		V_mu[k] = new_mu
		if k != K-1:
			other_components_expected = other_components_expected + U_S_expected_val[:, k]*V_mu[k] - U_S_expected_val[:, (k+1)]*V_mu[(k+1)]
	return V_mu, V_var


def outside_update_C_t(C_t_mu, C_t_var, N, base_resid_slice, tau_t_expected_val, cov, sum_cov_squared, base_cov_effects):
	...
	other_covariates = base_cov_effects - (cov[:, 0]*C_t_mu[0])

	# Loop through covariates
	for cov_num in range(num_cov):
		# Update mean and variance of Q(C) according to VI updates
		b_term = tau_t_expected_val*np.sum(cov[:, cov_num]*(base_resid_slice - other_covariates))
		a_term = 0.0 + tau_t_expected_val*sum_cov_squared[cov_num]
		new_var = 1.0/a_term
		new_mu = new_var*b_term
		C_t_mu[cov_num] = new_mu
		C_t_var[cov_num] = new_var

		if cov_num != num_cov-1:
			other_covariates = other_covariates + (cov[:, cov_num]*C_t_mu[cov_num]) - (cov[:, (cov_num+1)]*C_t_mu[(1+cov_num)])
	return C_t_mu, C_t_var
...
class SURGE_VI(object):
	...
	def fit(self, G, Y, cov, G_fe=None, z=None):
		...
		for vi_iter in range(self.max_iter):
			...
			if self.re_boolean:
				self.update_psi()
			# Used for both TAU AND ELBO calculations
			self.update_squared_residual_mat()
			self.update_tau_with_precomputed_squared_residual_matrix()
	...
	def verbose_printing(self, start_time, end_time, vi_iter):
		...
		print('##############')
		print('##############')
		self.runtimes.append(end_time-start_time)
		...
		if np.mod(vi_iter, 50) == 0 and vi_iter > 0:
			...
			np.savetxt(self.output_root + 'temp_iteration_runtimes.txt', np.asarray(self.runtimes), fmt="%s", delimiter='\n')
	def print_logo(self):
		print('*********************************************************')
		print('SURGE')
	...
	def update_V(self):
		...
		covariate_predicted = self.cov@self.C_mu
		...
		for test_index in range(self.T):
			...
			self.V_mu[:, test_index], self.V_var[:, test_index] = outside_update_V_t(V_mu_copy[:, test_index], V_var_copy[:, test_index], self.G[:, test_index], self.G_fe[:, test_index], self.Y[:, test_index], self.K, U_S_expected_val, U_S_squared_expected_val, self.F_mu[test_index], covariate_predicted[:, test_index], self.alpha_big_mu[:, test_index], self.gamma_v, tau_expected_val[test_index])
	...
	def update_C(self):
		...
		components_expected = self.U_mu@self.V_mu
		base_resid = self.Y - self.alpha_big_mu - self.G*components_expected
		base_cov_effects = self.cov@C_mu_copy

		# Loop through each test
		for test_index in range(self.T):
			# Update C for a particular test
			self.C_mu[:, test_index], self.C_var[:, test_index] = outside_update_C_t(C_mu_copy[:, test_index], C_var_copy[:, test_index], self.N, base_resid[:, test_index] - self.G_fe[:, test_index]*self.F_mu[test_index], tau_expected_val[test_index], self.cov, self.sum_cov_squared, base_cov_effects[:,test_index])
	...
	def update_tau_with_precomputed_squared_residual_matrix(self):
		self.tau_alpha = self.tau_alpha*0.0 + (self.N/2.0) + self.alpha_prior
		self.tau_beta = (np.sum(self.squared_residual_mat,axis=0)/2.0) + self.beta_prior
	def update_tau(self):
		###################
		# UPDATE tau
		###################
	...
	def update_elbo(self):
		...
		#data_likelihood_term_old = self.compute_elbo_log_likelihood_term()
		data_likelihood_term = self.compute_elbo_log_likelihood_term_with_precomputed_residual_matrix()
	...
	def compute_kl_divergence_of_U_S(self):
		...
		return kl_divergence
	def compute_elbo_log_likelihood_term_with_precomputed_residual_matrix(self):
		...
		term_c = np.sum(self.squared_residual_mat*tau_expected)/2.0
	...
	def compute_elbo_log_likelihood_term(self):
		...
		data_likelihood_term = term_a + term_b - term_c
		return data_likelihood_term
	def update_squared_residual_mat(self):
		# Other relevent expectations
		U_S = (self.U_mu)
		V_S = (self.V_mu)
		F_S = (self.F_mu)
		...
		component_squared_terms = U_S_squared@V_S_squared
		F_squared_terms = np.ones((self.N,1))@[F_S_squared]
		component_terms = U_S@V_S
		F_terms = np.dot(np.ones((self.N,1)),[F_S])
		covariate_terms = self.cov@self.C_mu
		squared_covariate_terms = np.square(self.cov)@C_squared
		alpha_squared_terms = np.square(self.alpha_big_mu) + self.alpha_big_var
		...
		squared_residual_mat = squared_residual_mat + (covariate_terms*covariate_terms - (np.square(self.cov)@np.square(self.C_mu)))
		squared_residual_mat = squared_residual_mat + np.square(self.G)*(component_terms*component_terms - (np.square(U_S)@np.square(V_S)))
		self.squared_residual_mat = squared_residual_mat
	...
	def initialize_variables(self):
		...
		self.psi_beta = np.ones(self.T)*self.beta_prior*.01 # Initialize random effects variance to be samller than residual variance (tau)
		...
		if self.re_boolean == True:
			...
			self.alpha_var = (np.zeros((self.I, self.T)) + 1.0)*.01  # Initialize to have same variance as that initialized by psi
		...
		self.sum_cov_squared = np.sum(np.square(self.cov),axis=0)
