...
def extract_matrix_from_market_format(input_file):
	f = open(input_file)
	head_count = 0
	prev_val = 1
	for line in f:
		...
		if head_count == 0:
			...
			rows = int(data[1])
			columns = int(data[0])
			matty = np.zeros((rows, columns))
			continue
		# Minus 1 for conversion from R to python 
		row_num = int(data[1]) - 1
		col_num = int(data[0]) - 1
		value = float(data[2])
		matty[row_num, col_num] = value
		if prev_val != row_num:
			if np.mod(row_num, 100) == 0:
				print(row_num)
			prev_val = row_num
	f.close()
	return matty

def extract_sparse_matrix_from_market_format(input_file):
	row_vec = []
	col_vec = []
	value_vec = []
	f = open(input_file)
	...
	for line in f:
		...
		if prev_val != row_num:
			...
			prev_val = row_num
	f.close()
	matty = csr_matrix((np.asarray(value_vec), (np.asarray(row_vec), np.asarray(col_vec))), shape=(rows, columns))
	return matty



def generate_scran_normalized_expression(adata, output_root):
	# Get ordred list of gene ids
	...
	raw_ordered_genes = raw_ordered_genes[non_zero_genes,:]
	...
	scran_log_counts = extract_sparse_matrix_from_market_format(output_root + 'raw_counts_log_norm_cpm_matrix_market_format.txt')
	
	return scran_log_counts, raw_ordered_genes


def generate_cluster_pseudobulk_expression_scran_qn_zscore(adata, gene_annotation_file, cluster_assignments, min_depth_threshold, genotyped_individuals_file, output_root):
	cluster_assignments2 = np.asarray(cluster_assignments)
	# Get dictionary list of genotyped individuals
	geno_indi_dicti = get_dictionary_list_of_genotyped_individuals(genotyped_individuals_file)
	...
	for pseudobulk_sample in ordered_pseudobulk_samples_raw:
		indi = pseudobulk_sample.split(':')[0]
		if indi in geno_indi_dicti and cells_per_indi[indi] > 2500:
			ordered_pseudobulk_samples.append(pseudobulk_sample)
			used_indis[indi] = 1
	ordered_pseudobulk_samples = np.asarray(ordered_pseudobulk_samples)
	print(str(len(used_indis)) + ' individuals of ' + str(len(geno_indi_dicti)) + ' genotyped indiviudals used in analysis')
	...
	#np.savetxt(output_root + 'raw_counts.txt', np.transpose((temp_raw2).astype(int)), fmt="%s", delimiter='\t')
	#os.system('Rscript scran_normalization.R ' + output_root + 'raw_counts.txt')
	scran_log_counts = extract_matrix_from_market_format(output_root + 'raw_counts_log_norm_cpm_matrix_market_format.txt')
	#np.save('temp_mat.npy', scran_log_counts)
	#scran_log_counts = np.load('temp_mat.npy')
	print('done')

	num_expressed_cells_per_gene = np.asarray(np.sum(adata.raw.X[:, non_zero_genes]>0,axis=0))[0,:]
	num_cells = adata.raw.X.shape[0]
	fraction_expressed_cells_per_gene = num_expressed_cells_per_gene/num_cells
	highly_expressed_genes = (num_expressed_cells_per_gene/num_cells)>.01
	scran_log_counts = scran_log_counts[:, highly_expressed_genes]
	raw_ordered_genes = raw_ordered_genes[highly_expressed_genes,:]

	# Generate raw cluster pseudobulk expression
	print(len(ordered_pseudobulk_samples))
	raw_cluster_pseudobulk_expression = generate_mean_cluster_pseudobulk_expression(scran_log_counts, ordered_pseudobulk_samples, cluster_assignments, raw_ordered_genes)
...
#######################################
# Run scran normalization of single cell data
#######################################
output_root = processed_expression_dir + 'scran_normalization_'
#scran_log_counts, scran_gene_names = generate_scran_normalized_expression(adata, output_root)


#######################################
# Put into AnnData object
#######################################
#temp_var = pd.DataFrame(data=scran_gene_names[:,1], index=scran_gene_names[:,0], columns=['gene_ids'])
#adata_scran = AnnData(X=scran_log_counts, obs=adata.obs, var=temp_var)
temp_h5_output_file = processed_expression_dir + 'scran_normalization.h5ad'
#adata_scran.write(temp_h5_output_file)

adata_scran = sc.read_h5ad(temp_h5_output_file)
...
sc.pp.highly_variable_genes(adata_scran)
...
# Save Expression PCs
#######################
pc_output_file = processed_expression_dir + 'scran_normalization_regress_batch_' + regress_out_batch + '_sc_expression_pcs.txt'
...
#######################
pc_pve_output_file = processed_expression_dir + 'scran_normalization_regress_batch_' + regress_out_batch + '_sc_expression_pcs_percent_variance_explained.txt'
...
# Save UMAP loadings
#######################
umap_output_file = processed_expression_dir + 'scran_normalization_regress_batch_' + regress_out_batch + '_sc_expression_umaps.txt'
np.savetxt(umap_output_file, adata_scran.obsm['X_umap'], fmt="%s", delimiter='\t', comments='')

#######################
# Save Covariate Info
#######################
covariate_output_file = processed_expression_dir + 'scran_normalization_regress_batch_' + regress_out_batch + '_cell_covariates.txt'
np.savetxt(covariate_output_file, adata_scran.obs, fmt="%s", delimiter='\t', header='\t'.join(adata_scran.obs.columns), comments='')

temp_h5_output_file = processed_expression_dir + 'scran_normalization_regress_batch_' + regress_out_batch + '_2.h5ad'
adata_scran.write(temp_h5_output_file)
...
#generate_cluster_pseudobulk_expression_scran_qn_zscore(adata, gene_annotation_file, adata.obs['individual_leiden_no_cap_clusters_' + str(resolution)], min_depth_threshold, genotyped_individuals_file, output_root)
