...
for chrom_num in range(1,23):
	...
	surge_anno_file = sldsc_processed_data_dir + 'surge_egenes_w_base_' + fdr + '.' + str(chrom_num) + '.annot'
	joint_surge_anno_file = sldsc_processed_data_dir + 'joint_surge_egenes_w_base_' + fdr + '.' + str(chrom_num) + '.annot'
	...
	for line in f:
		...
		if head_count == 0:
			head_count = head_count + 1
			# print header
			t.write('\t'.join(data[:4]) + '\t' + 'base\tsurge_eqtl_1\tsurge_eqtl_2\tsurge_eqtl_3\tsurge_eqtl_4\tsurge_eqtl_5\tsurge_eqtl_6\tsurge_eqtl_7\tsurge_eqtl_8\tsurge_eqtl_9\tsurge_eqtl_10\tstandard_eqtl\n')
			t_joint.write('\t'.join(data[:4]) + '\t' + 'base\tjoint_surge_eqtl\tstandard_eqtl\n')
			continue
		# error checking
		if len(data) != 57:
		...
		# Print header
		t.write('\t'.join(data[:4]) + '\t' + '1')
		t_joint.write('\t'.join(data[:4]) + '\t' + '1')
		joint_binary = '0.0'
		for latent_factor in range(1,11):
