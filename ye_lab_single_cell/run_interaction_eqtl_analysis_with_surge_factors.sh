...
qtl_expression_pc_file=$eqtl_input_dir"latent_factors.txt"
...
if false; then
python2 extract_expression_pc_factor_file_with_matched_number_of_components.py $surge_latent_factor_file $qtl_new_covariate_file $qtl_expression_pc_file $expression_pc_factor_file $qtl_covariate_remove_top_expression_pc_file
fi
...
if false; then
for job_number in $(seq 0 $(($num_jobs-1))); do 
	qtl_output_root=$output_stem"perm_False_expression_pc_interaction_eqtl_results_"$job_number"_"$num_jobs"_"
