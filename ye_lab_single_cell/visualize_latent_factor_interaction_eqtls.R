...
make_neg_log10_pvalue_qq_plots <- function(pvalue1) {
	neg_log_pvalue1 = -log10(pvalue1 + 1e-50)
	uniform_pvalues = runif(length(neg_log_pvalue1))
	neg_log_uniform_pvalues = -log10(uniform_pvalues + 1e-50)

	real_pvalues <- c(sort(neg_log_pvalue1))
	null_pvalues <- c(sort(neg_log_uniform_pvalues))

	df <- data.frame(real_pvalues=real_pvalues, null_pvalues=null_pvalues)

	p <- ggplot(df, aes(x=null_pvalues, y=real_pvalues)) + geom_point() +
		labs(x="Null", y="Real", title="Latent factor interaction eQTLs") + 
		geom_abline(color="blue") + 
		figure_theme()
	return(p)
}
...
output_file <- paste0(visualization_dir, "qqplot_of_lf_interaction_pvalues.pdf")
qq_plot <- make_neg_log10_pvalue_qq_plots(lf_interaction_pvalues)
