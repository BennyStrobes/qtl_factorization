...
def outside_update_U_n(U_mu, U_var, G_slice, G_fe_slice, Y_slice, K, V_S_expected_val, V_S_squared_expected_val, F_S_expected_val, covariate_predicted_slice, gamma_u, tau_expected_val, alpha_i_expected_val, k):
...
def outside_update_V_t(V_mu, V_var, G_slice, G_fe_slice, Y_slice, K, U_S_expected_val, U_S_squared_expected_val, F_S_t_expected_val, covariate_predicted_slice, alpha_t_mu, gamma_v, tau_t_expected_val, test_index, k):
	# Compute expectations on other components
	other_components_expected = (U_S_expected_val@V_mu) - U_S_expected_val[:, k]*V_mu[k]
	# Update variance of q(V|s=1)
	a_term = gamma_v[k] + (tau_t_expected_val*np.sum(np.square(G_slice)*U_S_squared_expected_val[:,k]))
	# Update mean of q(U|s=1)
...
class EQTL_FACTORIZATION_VI(object):
	...
	def fit(self, G, G_fe, Y, z, cov):
		...
		for vi_iter in range(self.max_iter):
			...
			if np.mod(vi_iter, 5) == 0 and vi_iter > 0:
				...
				np.save(self.output_root + 'temper_gamma_V.npy', self.gamma_V_alpha/self.gamma_V_beta)
	...
	def update_V(self, kk):
		...
		for test_index in range(self.T):
			V_update_data.append(outside_update_V_t(V_mu_copy[:, test_index], V_var_copy[:, test_index], self.G[:, test_index], self.G_fe[:, test_index], self.Y[:, test_index], self.K, U_S_expected_val, U_S_squared_expected_val, self.F_mu[test_index], covariate_predicted[:, test_index], self.alpha_big_mu[:, test_index], gamma_v, tau_expected_val[test_index], test_index, kk))
	...
	def update_gamma_V(self):
		...
		for k in range(self.K):
			...
			self.gamma_V_alpha[k] = self.ard_alpha_prior + (self.T/2.0)
			self.gamma_V_beta[k] = self.ard_beta_prior + (np.sum(V_squared_k_expected_val)/2.0)
	...
	def update_elbo(self):
		...
		kl_theta_v = self.compute_kl_divergence_of_gamma_v()
		kl_C = self.compute_kl_divergence_of_C()
		#kl_alpha = self.compute_kl_divergence_of_alpha()

		kl_divergence = kl_V_S + kl_U_S + kl_F_S + kl_tau + kl_theta_v + kl_C
	...
	def compute_kl_divergence_of_U_S(self):
		...
		kl_divergence = compute_kl_divergence_of_gaussian_fixed_variance(W_mu, W_var, self.gamma_v, self.K)
		return kl_divergence
	...
	def initialize_variables(self):
		...
		self.gamma_V_alpha = np.ones(self.K)*self.gamma_v
