...
def get_sample_names(tissues, gtex_expression_dir, sample_name_file, gtex_individual_information_file, gtex_outlier_samples):
	# Initialize arr
	samples = []
		...
		for line in f:
			if head_count == 0:
				...
				for indi_id in data[4:]:
					if indi_id + ':' + tissue not in gtex_outlier_samples:
						samples.append(indi_id + ':' + tissue)
					else:
						gtex_outlier_samples[indi_id + ':' + tissue] = 0
				continue
			break
		f.close()
	for key_name in gtex_outlier_samples.keys():
		if gtex_outlier_samples[key_name] == 1:
...
def compute_md(expr):
	...
	for samp_num in range(nn):
		...
		mds.append(md)
		pvalues.append(pval)
	return np.asarray(mds), np.asarray(pvalues)


def identify_gtex_outlier_samples_based_on_expr_pcs(covariate_file):
	cov_raw = np.transpose(np.loadtxt(covariate_file, dtype=str, delimiter='\t'))
	cov = cov_raw[1:,1:].astype(float)
	sample_names = cov_raw[0,1:]
	cov_names = cov_raw[1:,0]
	valid_covs = []
	for i, cov_name in enumerate(cov_names):
		if cov_name.startswith('PC'):
			valid_covs.append(i)
	valid_covs = np.asarray(valid_covs)

	cov_filtered = cov[valid_covs,:]

	mds, md_pvalues = compute_md(cov_filtered)

	z_score_mds = (mds - np.mean(mds))/np.std(mds)

	samples_to_remove = sample_names[z_score_mds > 4.0]

	samples_to_remove_dicti = {}

	for sample_name in samples_to_remove:
		samples_to_remove_dicti[sample_name] = 1

	return samples_to_remove_dicti

def identify_gtex_outlier_samples(expr_file):
	raw_expr_data = np.loadtxt(expr_file, dtype=str,delimiter='\t')
	sample_names = raw_expr_data[0,1:]
	gene_names = raw_expr_data[1:,0]
	expr = raw_expr_data[1:,1:].astype(float)

	mds, md_pvalues = compute_md(expr)
	pdb.set_trace()
...
# Run outlier detection to identify gtex samples that are outliers
covariate_file = processed_data_dir + 'covariates.txt'
gtex_outlier_samples = identify_gtex_outlier_samples_based_on_expr_pcs(covariate_file)




# Now remove outlier samples and re-process


# Update output file to reflect this data is removed of outliers
processed_data_dir = processed_data_dir + 'outliers_removed_'



# Extract file of sample names
sample_name_file = processed_data_dir + 'sample_names.txt'
sample_names, sample_to_index, individual_covariates = get_sample_names(tissues, gtex_expression_dir, sample_name_file, gtex_individual_information_file, gtex_outlier_samples)
