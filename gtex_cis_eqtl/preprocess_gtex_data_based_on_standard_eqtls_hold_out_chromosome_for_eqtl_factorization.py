...
def extract_eqtl_factorization_tests(cross_tissue_eqtl_results_file, cross_tissue_genome_wide_sig_results_file, num_genes, held_out_chromosome):
	held_out_chromosome_string = 'chr' + held_out_chromosome + '_'
	dicti = {}
	binary_arr = []
	temp_dicti = {}
	...
	for line in f:
		...
		if head_count == 0:
			head_count = head_count + 1
			continue
		if test_name in dicti and variant_id.startswith(held_out_chromosome_string) == False:
			binary_arr.append(1)
		else:
			binary_arr.append(0)
	f.close()
...
held_out_chromosome = sys.argv[3]
...
test_eqtl_dicti, test_eqtl_binary_arr = extract_eqtl_factorization_tests(latent_factor_interaction_eqtl_results_file, latent_factor_interaction_eqtl_genome_wide_sig_results_file, num_genes, held_out_chromosome)

# Generate eqtl factorization test names file
all_test_names_file = working_dir + 'all_tests.txt'
eqtl_factorization_test_names_file = working_dir + 'eqtl_factorization_standard_eqtl_' + str(num_genes) + '_hold_out_chromosome_' + held_out_chromosome + '_input_test_names.txt'
...
eqtl_factorization_expression_file = working_dir + 'eqtl_factorization_standard_eqtl_' + str(num_genes) + '_hold_out_chromosome_' + held_out_chromosome + '_input_expression.txt'
...
eqtl_factorization_genotype_file = working_dir + 'eqtl_factorization_standard_eqtl_' + str(num_genes) + '_hold_out_chromosome_' + held_out_chromosome + '_input_genotype.txt'
...
covariate_with_intercept_file = working_dir + 'cross_tissue_eqtl_' + str(num_genes) + '_hold_out_chromosome_' + held_out_chromosome + '_covariate_w_intercept_input.txt'
